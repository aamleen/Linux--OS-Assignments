...
void handlerST(int signo){
  unsigned ra, rd;
  __asm__ __volatile__ ("rdtsc" : "=a"(ra), "=d"(rd));
  unsigned long long cpucount= ((unsigned long long)ra)|(((unsigned long long)rd)<<32) ;
...
int main(int argc, char const *argv[]){
  ...
  if(sigaction(SIGTERM,&sActionTerm,NULL)==-1){
    perror("Error in STHandler ");
    return 0;
  }
